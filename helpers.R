# ECG plotter helpers script for ion channel interactomes
#
...
#
# This is pretty close to the original version. We just did some minor changes.
...
make_ecg_plot <- function(vct_snp_p,
                          vct_snp_beta="",
                          vct_snp_se="",
                          vct_snp_info,
  ...
  dfecg <- cbind(as.data.frame(df_ecg_stats), t(vct_snp_p))
  ...
  ecgplot <- ggplot(dfecg,aes(x=time))+
    geom_line(data=dfecg,colour="red", aes(x = time, y= logP ,
                                           ...
                                           group=1  ), size = 2)  +
    geom_line(data=dfecg,aes(x = time,y=mean0/Pf),colour="black", size = 2) +
    scale_color_manual(name="Groups",values=c("red", "blue"))+
    ylab("Signed -log10(P-value)") + xlab("Time (ms)") +
    labs(title=vct_snp_info$SNP, subtitle=vct_snp_info$Gene) + theme_grey(base_size = 22)
